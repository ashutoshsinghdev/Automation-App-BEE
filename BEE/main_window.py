...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setupUi(self)  
        for btn in [self.newChat, self.chatHis, self.dashboard, self.settings, self.pushButton_3]:
...
#chatmanager:-
        self.chat = ChatManager(self.chatLayout)
#notification card
        self.chat.add_notification(#it is a function of chat manager.
            "Battery Low",
        "Battery has fallen below 20%."
            )
# input card
        card = self.chat.add_input_card("Enter Reminder Time", "e.g. 8:00 PM")
        card.submitted.connect(lambda text: print(f"User entered: {text}"))
        card.submitted.connect(card.deleteLater)  # remove card after submit

# list card
        items = [
    {"item": "Rent",    "price": 5000},
    {"item": "Tiffin",  "price": 2000},
    {"item": "Grocery", "price": 800}
]

        list_card = self.chat.add_list_card("Order Summary", items)
        list_card.confirmed.connect(lambda items: print("Ordered items are:", [i["item"] for i in items]))
        list_card.confirmed.connect(list_card.deleteLater)

# payment card
        pay_card = self.chat.add_payment_card("Arjun", 5000, "arjun@oksbi")
        pay_card.confirmed.connect(lambda name, amt, upi: print(f"Pay ₹{amt} to {name} though upi id:{upi}"))
        pay_card.cancelled.connect(pay_card.deleteLater)
#to open the pg:-
